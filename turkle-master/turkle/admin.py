...
from .models import ActiveUser, ActiveProject, Batch, Project, TaskAssignment, UserEvaluation, ProjectManager, BatchManager
...
class BatchForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['csv_file'].help_text = 'You can Drag-and-Drop a CSV file onto this ' + \
            ...
            '\n Make sure your CSV file has a field named  exactly "calibration_answer"' + \
            '\n And for tasks with no calibration answer for them have the value "0"' + \
            '\n And for tasks with calibration answers for the have the values saved as a JSON'
        ...
        self.fields['is_majority_voting'].label = 'Majority voting'
        self.fields['majority_count'].label = 'Minimum Users'
        self.fields['majority_count'].help_text = 'The minimum users needed for majority voting'
...
class BatchAdmin(AjaxAutocompleteListFilterModelAdmin):
    ...
    list_display = (
        ...
        'stats', 'download_input', 'download_csv', 'majority_count'
        )
    ...
    def get_queryset(self, request):
        queryset = BatchManager.get_queryset(self, request)
        return queryset
        ...
        if not obj:
            ...
            return (
                ...
                ('Majority voting Settings', {
                    'fields': ('is_majority_voting', 'majority_count')
                }),
                ('Permissions', {
                    'fields': (
...
class ProjectForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['evaluation_script'].widget = CustomButtonFileWidget(attrs={
            'class': 'hidden',
        })
        self.fields['evaluation_threshold'].help_text = 'the ratio of calibrated tasks to total tasks'
        self.fields['calibration_limit'].help_text = 'the minimum tasks to check the user\'s rating'
        self.fields['calibration_rating'].help_text = 'the minimum rating for calibration tasks'
...
class ProjectAdmin(GuardedModelAdmin, AjaxAutocompleteListFilterModelAdmin):
    ...
    def get_queryset(self, request):
        queryset = ProjectManager.get_queryset(self, request)
        return queryset
    ...
    def get_fieldsets(self, request, obj=None):
        if not obj:
            ...
            return (
                ...
                ('Evaluation', {
                    'fields': ('evaluation_script', 'evaluation_threshold',
                               'calibration_limit', 'calibration_rating')
                }),
    ...
    def get_list_display(self, request):
        if request.user.has_perm('turkle.add_batch'):
            return ('name', 'filename', 'updated_at', 'active', 'stats', 'publish_tasks', 'evaluation_threshold',
                    'calibration_limit', 'calibration_rating', 'evaluation_script')
        else:
            return ('name', 'filename', 'updated_at', 'active', 'stats', 'evaluation_threshold',
                    'calibration_limit', 'calibration_rating', 'evaluation_script')
...
class UserEvaluationAdmin(ViewOnlyAdminMixin, AjaxAutocompleteListFilterModelAdmin):
    list_display = ('id_user', 'id_batch', 'get_project', 'rating', 'count', 'updated_at')
    list_per_page = 20
    preserve_filters = False
    list_filter = ('id_batch__project',)
    autocomplete_list_filter = ('id_batch', 'id_user')

    @admin.display(ordering='batch__project', description='Project')
    def get_project(self, obj):
        return obj.id_batch.project
