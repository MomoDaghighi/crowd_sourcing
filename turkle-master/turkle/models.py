...
def deactivate_user(user, project):
    groups = set(user.groups.all()).intersection(set(get_groups_with_perms(project)))
    for group in groups:
...
class UserEvaluation(models.Model):
    ...
    id_user = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name='user', on_delete=models.CASCADE)
    id_batch = models.ForeignKey('Batch', verbose_name='batch', on_delete=models.CASCADE)
    updated_at = models.DateTimeField(auto_now=True, db_index=True)
    rating = models.FloatField(default=0)
    count = models.IntegerField(default=0)

    def deactivate_user_with_low_rating(self, user):
        batch = self.id_batch
        project = batch.project
        calibration_limit = project.calibration_limit
        calibraion_rating = project.calibration_rating

        if self.count >= calibration_limit:
            if self.rating < calibraion_rating:
                deactivate_user(self.id_user, project)
                return 1
        return 0
...
class Task(models.Model):
    ...
    def calibration_rate_for_the_task(self, user_answer, user):
        calibration_answer = self.input_csv_fields['calibration_answer']
        batch = self.batch
        project = batch.project
        if project.evaluation_script and calibration_answer != '0':
            try:
                rating = subprocess.run('python3 {} "{}" "{}"'.format(project.evaluation_script, calibration_answer, user_answer)
                                        , shell=True, capture_output=True, text=True).stdout

                if UserEvaluation.objects.filter(id_batch=batch, id_user=user):
                    # user has previous records of evaluation for the batch
                    user_evaluation = UserEvaluation.objects.filter(id_batch=batch, id_user=user).first()
                    new_rating = ((user_evaluation.rating * user_evaluation.count) + float(rating)) / (
                            user_evaluation.count + 1)
                    user_evaluation.count += 1
                else:
                    new_rating = rating
                    user_evaluation = UserEvaluation()
                    user_evaluation.count = 1
                    user_evaluation.id_user = user
                    user_evaluation.id_batch = batch

                user_evaluation.updated_at = datetime.datetime.now()
                user_evaluation.rating = new_rating
                user_evaluation.save()
            except:
                logger.error('check the error')
...
class TaskAssignment(models.Model):
    ...
    assigned_to = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name='assigned_to', db_index=True, null=True,
    ...
    def work_time_in_seconds(self):
        ...
        if self.completed:
            ...
            raise ValueError(
                ...
                self.id)
    @staticmethod
    def check_majority_voting(done_taskassignments, task_id):
        tasks = done_taskassignments.values('answers').order_by().annotate(answers_count=Count('answers'))
        maxval = max(tasks, key=lambda x: x['answers_count'])
        batch = Task.objects.get(id=task_id).batch
        minimum = batch.majority_count
        if maxval['answers_count'] >= minimum:
...
class Batch(TaskAssignmentStatistics, models.Model):
    ...
    majority_count = models.IntegerField(null=True, blank=True)
    ...
    def available_tasks_for(self, user):
        ...
        hs = Task.objects.filter(batch_id=self, completed=False).values()
    ...
    def none_calibration_task_ids(self, available_tasks):
        none_calibration_tasks = available_tasks.filter(input_csv_fields__contains='"calibration_answer": "0"')
        return none_calibration_tasks.values_list('id', flat=True)

    def calibration_task_ids(self, available_tasks):
        calibration_tasks = available_tasks.exclude(input_csv_fields__contains='"calibration_answer": "0"')
        if len(calibration_tasks) == 0:
            return available_tasks.values_list('id', flat=True)
        return calibration_tasks.values_list('id', flat=True)
...
class Project(TaskAssignmentStatistics, models.Model):
    ...
    updated_by = models.ForeignKey(settings.AUTH_USER_MODEL, null=True,
                                   ...
                                   on_delete=models.CASCADE, verbose_name='updated_by')
    evaluation_script = models.FileField(verbose_name='evaluation script', null=True, blank=True)
    evaluation_threshold = models.FloatField(validators=[MinValueValidator(0.0), MaxValueValidator(1.0)], default=0)
    calibration_limit = models.IntegerField(null=True)
    calibration_rating = models.FloatField(validators=[MinValueValidator(0.0), MaxValueValidator(1.0)], default=0)
...
class ProjectManager(models.Manager):
    def get_queryset(self, request):
        ...
        checker = TurklePermissionChecker(request.user)
        checker.prefetch_perms(projects)

        project_list = [p for p in projects if checker.has_perm('can_work_on', p)]
        project_query = Project.objects.filter(Q(id__in=[p.id for p in project_list]) | Q(created_by=request.user))
...
class BatchManager(models.Manager):
    def get_queryset(self, request):
        ...
        project_list = [p for p in projects if checker.has_perm('can_work_on', p)]
        batches_list = [b for b in batches if checker.has_perm('can_work_on_batch', b)]

        batch_query = Batch.objects.filter(Q(project__created_by=request.user) |
                                           Q(created_by=request.user) |
                                           Q(id__in=[b.id for b in batches_list]))
        # Q(project__in=[p.id for p in project_list]) |
        if request.user.is_superuser:
